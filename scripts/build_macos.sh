...
if [ "$CREATE_APP_BUNDLE" = true ]; then
    ...
    BINARY_PATH="install/bin/reflection"
    ...
    mkdir -p Reflection.app/Contents/{MacOS,Resources,Frameworks}
    
    # Copy binary
    cp "$BINARY_PATH" Reflection.app/Contents/MacOS/
    
    # Copy resources if they exist
    if [ -d "install/share" ]; then
        cp -r install/share Reflection.app/Contents/Resources/
    fi
...
echo -e "  Direct: ${BLUE}./install/bin/reflection${NC}"
